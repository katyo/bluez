...
#define XR_DUMP(buffer, len)                               \
  ...
  do {                                                     \
    for (int i = 0; i < len; i++) {                        \
      ...
      fprintf(stderr, "%02" PRIu8 " ", buffer[i]);				 \
    }                                                      \
    fprintf(stderr, "\n");                                 \
  } while (0)

#define BT_FW_PATH                          "/lib/firmware"
#define BT_FW_NAME                          "fw_xr829_bt.bin"
#define BT_FW_PATH_NAME                     BT_FW_PATH "/" BT_FW_NAME
...
static int32_t xr_raw_write(int type, uint8_t *data, uint32_t len)
{
	...
	psend = data;
	lsend = len;
	hdr = (cmd_header_t *)buffer;
	ack = (cmd_ack_t *)buffer;
	...
	XR_DBG("%s, type: %d, write len: %5" PRIu32 ", ret: %5zd, time: %6" PRIu64 "us,"
				 " read len: %2zu, ret: %2zd, %6" PRIu64 "us",
			   __func__, type, lsend, ret_w, t1 - t0, MB_CMD_HEADER_SIZE, ret_r, t3 - t2);

	p = (uint8_t *)memsearch(buffer, MB_CMD_HEADER_SIZE, (uint8_t *)"BROM", 4);
	...
	if (ack->h.flags & CMD_HFLAG_ERROR) {
		...
		XR_ERR("%s: resp error flag, type %" PRIu8, __func__, ack->err);
		return -ack->err;
	}
	...
	if (ack->h.flags & CMD_HFLAG_ACK) {
		...
		if (ack->h.payload_len != 0) {
			XR_ERR("%s: data payload len %" PRIu32 " != 0", __func__, ack->h.payload_len);
			return -1;
		}
	}
...
static int32_t cmd_sync_uart(void)
{
	...
	do {
		XR_DBG("uart sync count:%" PRIu32 ".", cnt);
		tcflush(vnd_userial.fd, TCIOFLUSH);
...
static int32_t cmd_sync_baud(uint32_t lcr)
{
	...
	do {
		XR_DBG("%s count:%" PRIu8 ".", __func__, cnt);
...
static int32_t cmd_set_pc(uint32_t pc)
{
	...
	cmd->val   = pc;
	XR_DBG("set pc %" PRIx32 ", val %" PRIx32, pc, cmd->val);
...
static int32_t load_btfirmware(void)
{
	...
	if (data == NULL) {
		XR_DBG("failed to alloc %" PRIu32 " byte memory.", section);
		fclose(fwfile_fd);
		return -1;
	}
	...
	XR_DBG("Firmware run from address 0x%08" PRIX32, BT_FW_JUMP_ADDR);
...
static int hci_cmd_handle(const uint8_t *cmd, uint32_t cmd_len, uint32_t event_len)
{
	...
	XR_DUMP(cmd, (int)cmd_len);

	if (read_hci_event(vnd_userial.fd, buffer, event_len) != (int)event_len) {
		XR_ERR("Event read error");
		return -1;
	}
	XR_DBG("Received event");
	XR_DUMP(buffer, (int)event_len);
	return 0;
}
...
int xr_init(int fd, int init_speed, int speed, struct termios *ti)
{
	...
	if (cmd_sync_uart() < 0)
		goto END;
	XR_DBG("set bandrate to %d.", speed);
	if (cmd_sync_baud(((speed) | (3<<24))) < 0)
		goto END;
	if (load_btfirmware() < 0)
		goto END;
	XR_DBG("bt firmware is running....");

	XR_DBG("set baudrate to %d", init_speed);
	set_speed(vnd_userial.fd, vnd_userial.ti, init_speed);
