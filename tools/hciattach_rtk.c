...
#define RS_DUMP(buffer, len)																 \
	do {                                                       \
		...
		for (int i = 0; i < len; i++) {													 \
			...
			fprintf(stderr, "%02" PRIx8 " ", buffer[i]);					 \
		}																												 \
		fprintf(stderr, "\n");																	 \
	} while (0)
...
{
	if ((int)skb->data_len > len) {
		skb->data_len = len;
	} else {
		RS_ERR("Error: skb->data_len(%" PRIu32 ") < len(%d)", skb->data_len, len);
	}
}
...
*/
static void h5_unslip_one_byte(struct rtk_h5_struct *h5, uint8_t byte)
{
...
static void rtk_send_pure_ack_down(int fd)
{
	...
	if (write(fd, nskb->data, nskb->data_len) != 0x8) {
		RS_ERR("%s: Write error", __func__);
	}
...
static void hci_event_cmd_complete(struct sk_buff *skb)
{
	...
	RS_DBG("receive hci command complete event with command: %" PRIx16, opcode);
	if (DBG_ON) {
		RS_DBG("Dump event data with event header (header size %zu):", sizeof(struct hci_ev_cmd_complete));
		RS_DUMP(skb->data, (int)skb->data_len);
	}
	...
	case HCI_VENDOR_CHANGE_BDRATE:
		status = skb->data[0];
		RS_DBG("Change BD Rate with status:%" PRIx8, status);
	...
	case HCI_VENDOR_READ_CHIP_TYPE:
		...
		status = skb->data[0];
		RS_DBG("Read RTK CHIP TYPE with Status:%" PRIx8, status);
		if (0 == status) {
	...
	case HCI_VENDOR_READ_RTK_ROM_VERISION:
		...
		status = skb->data[0];
		RS_DBG("Read RTK rom version with Status:%" PRIx8, status);
		if (0 == status)
...
static void hci_recv_frame(struct sk_buff *skb)
{
	...
	if (rtk_h5.link_estab_state == H5_SYNC) {  /* sync */
		if (!memcmp(skb->data, h5sync, 2)) {
			RS_DBG("Get Sync Pkt");
			if (write(serial_fd, &h5_sync_resp_pkt, 0x8) != 0x8) {
				RS_ERR("%s: Write error", __func__);
			}
		...
		if (!memcmp(skb->data, h5sync, 0x2)) {
			...
			if (write(serial_fd, &h5_conf_resp_pkt_to_Ctrl, 0x8) != 0x8) {
				RS_ERR("%s: Write error", __func__);
			}
		...
		RS_DBG("rtk_patch.nRxIndex %d", rtk_patch.nRxIndex);
...
static int h5_recv(struct rtk_h5_struct *h5, void *data, int count)
{
	...
	while (count) {
		...
		case H5_W4_HDR:
			...
			if ((~(h5->rx_skb->data[0] + h5->rx_skb->data[1] +
						h5->rx_skb->data[2]) & 0xff) != h5->rx_skb->data[3]) {
...
static void h5_tshy_sig_alarm(int sig)
{
	...
	if (retries < h5_max_retries) {
		...
		retries++;
		nskb = h5_prepare_pkt(&rtk_h5, h5sync, sizeof(h5sync), H5_LINK_CTL_PKT);
		len = write(serial_fd, nskb->data, nskb->data_len);
		RS_DBG("3-wire sync pattern resend: %d, len: %d", retries, len);
		skb_free(nskb);
...
static void h5_tconf_sig_alarm(int sig)
{
	...
	if (retries < h5_max_retries) {
		...
		retries++;
		nskb = h5_prepare_pkt(&rtk_h5, h5conf, 3, H5_LINK_CTL_PKT);
		len = write(serial_fd,  nskb->data, nskb->data_len);
...
static void h5_tinit_sig_alarm(int sig)
{
	...
	if (retries < h5_max_retries) {
		...
		if (rtk_h5.host_last_cmd) {
			...
			len = write(serial_fd, rtk_h5.host_last_cmd->data, rtk_h5.host_last_cmd->data_len);
			RS_DBG("3-wire change baudrate resend: %d, len: %d", retries, len);
			alarm(1);
			return;
		} else {
...
static void h5_tpatch_sig_alarm(int sig)
{
	...
	if (retries < h5_max_retries) {
		...
		if (rtk_h5.host_last_cmd) {
			...
			len = write(serial_fd, rtk_h5.host_last_cmd->data, rtk_h5.host_last_cmd->data_len);
			RS_DBG("3-wire download patch re send: %d, len: %d", retries, len);
		}
		retries++;
		alarm(3);
		return;
...
static int hci_download_patch(int dd, int index, uint8_t *data, int len, struct termios *ti)
{
  ...
  nskb = h5_prepare_pkt(&rtk_h5, hcipatch, len+4, HCI_COMMAND_PKT); /* data: len + head: 4 */
	...
	RS_DBG("hci_download_patch nTxIndex:%d nRxIndex: %d, len: %d",
				 rtk_patch.nTxIndex, rtk_patch.nRxIndex, len);
...
{
	uint8_t bytes[257] = {0};
	uint8_t buf[257] = {0x01, 0x20, 0xfc, 00};
	int cur_index;
	int ret_Index;
	size_t total_len;
	uint16_t w_len;
	...
	RS_DBG("h4 write success with len: %d", w_len);
	...
	if (DBG_ON) {
		...
		RS_DBG("h4 read success with len: %d", res);
		for (i = 0; i < 8; i++) {
	...
	if ((0x04 == bytes[0]) && (0x20 == bytes[4]) && (0xfc == bytes[5])) {
		ret_Index = bytes[7];
		rstatus = bytes[6];
		RS_DBG("---->ret_Index:%d, ----->rstatus:%d", ret_Index, rstatus);
		if (0x00 != rstatus) {
...
static uint32_t rtk_parse_config_file(uint8_t *config_buf, size_t filelen, uint8_t bt_addr[6])
{
	...
	if (config_len != filelen - sizeof(struct rtk_bt_vendor_config)) {
		RS_ERR("config len(%" PRIu16 ") is not right(%zu)", config_len, filelen - sizeof(struct rtk_bt_vendor_config));
		return 0;
	}
...
*
*/
static void rtk_write_btmac2file(uint8_t bt_addr[6])
{
	int fd;
	...
	if (fd > 0) {
		...
		sprintf(addr, "%2" PRIx8 ":%2" PRIx8 ":%2" PRIx8 ":%2" PRIx8 ":%2" PRIx8 ":%2" PRIx8,
						bt_addr[0], bt_addr[1], bt_addr[2], bt_addr[3], bt_addr[4], bt_addr[5]);
		addrlen = strlen(addr);

		if (write(fd, addr, addrlen) != addrlen) {
			RS_ERR("write file error: %s", BT_ADDR_FILE);
		}
		close(fd);
	} else {
		RS_ERR("open file error: %s", BT_ADDR_FILE);
	}
}
...
static int rtk_get_bt_config(uint8_t **config_buf, uint32_t *config_baud_rate, patch_info *info)
{
	...
	if (file == NULL) {
		...
		if (fscanf(file, "%2x:%2" SCNx32 ":%2" SCNx32 ":%2" SCNx32 ":%2" SCNx32 ":%2" SCNx32,
							 (int *)&bt_addr[0], (uint32_t *)&bt_addr[1],
							 (uint32_t *)&bt_addr[2], (uint32_t *)&bt_addr[3],
							 (uint32_t *)&bt_addr[4], (uint32_t *)&bt_addr[5]) != 6) {
			RS_ERR("Invalid bt btaddr file");
		}
		...
		RS_DBG("BT MAC IS: %02" PRIX8 ":%02" PRIX8 ":%02" PRIX8 ":%02" PRIX8 ":%02" PRIX8 ":%02" PRIX8,
					 bt_addr[0], bt_addr[1], bt_addr[2], bt_addr[3], bt_addr[4], bt_addr[5]);
	}
...
GET_CONFIG:
	...
	*config_buf = malloc(filelen);
	if (*config_buf == NULL) {
		RS_DBG("malloc buffer for config file fail(%zu)", filelen);
		close(fd);
		return -1;
	}
...
static int rtk_vendor_change_speed_h5(int fd, uint32_t baudrate)
{
	...
	alarm(1);
	if (write(fd, cmd_change_bdrate->data,
						cmd_change_bdrate->data_len) !=
			cmd_change_bdrate->data_len) {
		RS_ERR("%s: Write error", __func__);
	}
...
static void rtk_download_fw_config(int fd, uint8_t *buf, size_t filesize, int is_sent_changerate, int proto, struct termios *ti)
{
	...
	RS_DBG("iEndIndex:%d  iLastPacketLen:%d iAdditionpkt:%d", iEndIndex, iLastPacketLen, iAdditionPkt);
	...
	for (i = 0; i <= iTotalIndex; i++) {
		...
		if (proto == HCI_UART_H4) {
			...
			if ((iCurIndex != i) && (i != rtk_patch.nTotal)) {
				...
				RS_DBG("index mismatch i:%d iCurIndex:%d, patch fail", i, iCurIndex);
				return;
			}
...
static void rtk_speed_to_uart_speed(uint32_t rtk_speed, uint32_t *uart_speed)
{
	...
	*uart_speed = 115200;
	...
	for (i = 0; i < (int)(sizeof(baudrates) / sizeof(baudrate_ex)); i++) {
...
static void rtk_get_eversion_timeout(int sig)
{
	...
	if (retries < h5_max_retries) {
		RS_DBG("patch timerout, retry:");
		if (rtk_h5.host_last_cmd) {
			if (write(serial_fd, rtk_h5.host_last_cmd->data,
								rtk_h5.host_last_cmd->data_len) !=
					rtk_h5.host_last_cmd->data_len) {
				RS_ERR("%s: Write error", __func__);
			}
...
static void rtk_get_eversion(int dd)
{
	...
	if (write(dd, nskb->data, nskb->data_len) != nskb->data_len) {
		RS_ERR("%s: Write error", __func__);
	}
...
FETCH_DONE:
	...
	RS_DBG("final_speed %d", final_speed);
...
{
	RS_DBG("Realtek hciattach version %s", RTK_VERSION);
