...
static size_t H4Protocol_Send(uint8_t type, const uint8_t* data, size_t length)
{
	...
	} else if (ret < (ssize_t)length + 1) {
		SPRD_ERR("%s: %zd / %zu bytes written - something went wrong...", __func__, ret, length + 1);
	}
...
static void hw_core_cback(void *p_mem)
{
	...
	SPRD_DBG("%s hw_core_cback response: [0x%04" PRIX16 ", 0x%04" PRIX16 ", 0x%02" PRIX8 "]",
					 __func__, opcode, mode, status);
...
static void hw_rf_cback(void *p_mem)
{
	...
	SPRD_DBG("%s hw_rf_cback response: [0x%04" PRIX16 ", 0x%04" PRIX16 ", 0x%02" PRIX8 "]",
					 __func__, opcode, mode, status);
...
static void marlin3_pskey_cback(void *p_mem)
{
	...
	SPRD_DBG("Bluetooth Firmware Node: %04" PRIX16
					 " Date: %04" PRIx16 "-%02" PRIx8 "-%02" PRIx8,
					 node, year, month, day);
